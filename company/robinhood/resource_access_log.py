...
user_8:[100,100]
'''
def user_min_max(logs):
  res = dict()
  for log in logs:
    if log[1] in res:
      log_min = res[log[1]][0]
      log_max = res[log[1]][1]
      log_min = min(log_min, int(log[0]))
      log_max = max(log_max, int(log[0]))
      res[log[1]] = [log_min, log_max]
    else:
      res[log[1]] = [int(log[0]), int(log[0])]
  return res
...
print(user_min_max(logs1))
print(user_min_max(logs2))
...
def most_resource(logs):
  times_by_user = defaultdict(list)
  for time,_,resource in logs:
    time = int(time)
    times_by_user[resource].append(time)
  max_resource = None
  max_access = -1
  for resource in times_by_user.keys():
    time_seq = times_by_user[resource]
    time_seq.sort()
    i, j = 0, 0
    accesses = 0
    while i <= j < len(time_seq):
      while j < len(time_seq) and time_seq[j] - time_seq[i] <= 300:
        accesses = max(accesses, j - i + 1)
        j += 1
      while j < len(time_seq) and time_seq[j] - time_seq[i] > 300:
        i += 1
  ...
  return (max_resource, max_access)

print(most_resource(logs1))
'''
Question 3 -
...
For example, of 8 total users, 4 users have resource_3 as a first visit (user_1, user_2, user_3, user_5), 2 users have resource_1 as a first visit (user_6, user_22), 1 user has resource_2 as a first visit (user_7), 
and 1 user has resource_6 (user_8) so the possible next steps for START are resource_3 with probability 4/8, resource_1 with probability 2/8, and resource_2 and resource_6 with probability 1/8.
