 ...
 *
 * @returns Maximum about we can rob
 */
function rob(nums: number[]): number {
  ...
  const maxArr = new Array(nums.length).fill(0);
  // maxArr[i] is max amount you can get until current house.
  // maxArr[i] = (nums[i] + maxArr[i-2]) || maxArr[i-1]
  maxArr[0] = nums[0];
  maxArr[1] = Math.max(maxArr[0], nums[1]);
