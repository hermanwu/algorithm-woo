...
function accountsMerge(accounts: string[][]): string[][] {
  ...
  function dfs(list, email) {
    list.push(email);

    for (let next of graph.get(email).values()) {
  ...
  for (let email of emailToNameMap.keys()) {
    const list = [];
    if (visited.has(email) === false) {
      visited.add(email);

      dfs(list, email);

      list.sort((a, b) => a - b);
      results.push([emailToNameMap.get(email), ...list]);
    }
  }
